...
## Session 13 — ORCHESTRATOR IDEMPOTENCY FIX (COMPLETE)
- **Docker sandbox deployable (`backend/sandbox/`)**: `sandbox_server.py` can now run as a container — `docker compose up -d --build` on a DO droplet (RAILPACK on Railway ignores Dockerfiles, so no conflict). Fixed two containerization bugs in `sandbox_server.py`/compose: (1) `mkdtemp` 0700 root dirs were unreadable by execution containers running as uid 1000 — now `chmod 755` dir + `644` files; (2) temp bind-mount paths must exist on the host — shared `/opt/sandbox-tmp` mounted at the same path in host+container, `TMPDIR=/opt/sandbox-tmp`, plus host docker socket mount, `cap_drop ALL` + `no-new-privileges`, healthcheck, and a `pre-pull` compose service for the 11 language images. Setup: `cp .env.example .env` → set `SANDBOX_API_KEY` → `docker compose up -d --build`, then point the admin panel Code Exec settings at `http://<droplet-ip>:8080` with the same key.
- **HTTPS via Caddy (`backend/sandbox/`)**: sandbox now sits behind a Caddy reverse proxy (same compose file) that auto-issues a Let's Encrypt cert — point `SANDBOX_DOMAIN` (e.g. `sandbox.algokube.in` A record → droplet IP) at the droplet and use `https://<SANDBOX_DOMAIN>` as the admin Sandbox Server URL. The sandbox service is internal-only (`expose: 8080`, no public port); 80/443 are the only open ports. No manual cert management.
- **Sandbox URL auto-normalization (`code_execution_settings.py` + `main.py`)**: bare IPs/hostnames (e.g. `165.232.179.64`) are now prefixed with `http://` on save, at runtime sync, and at startup restore, so the admin "Test connection" can't fail with "Request URL is missing an 'http://' or 'https://' protocol" anymore.
- **Prod re-run blocker fixed (`seed_production.py`)**: the legacy batch seeder `seed_interview_engineering_courses.py` was still in the MANIFEST for the 5 interview/architecture courses. Re-running it against an algomaster-restructured DB violated the globally-unique `lessons.slug` constraint (e.g. `duplicate key ... (slug)=(database-per-service)`), so every prod maintenance run FAILED. Those 5 courses are fully owned by the algomaster restructure (`ALGOMASTER_COURSES`, which creates each course from scratch via `seed_algomaster_curriculum.py`), so the stale MANIFEST entries were removed and the legacy batch seeder is retired.
- **`--only` validation updated**: `unknown = only - known` now accepts `ALGOMASTER_COURSES` slugs too (they're valid `--only` targets even though not in MANIFEST) — the algomaster step handles them end-to-end.
- **Prod scalar `content_blocks` crash fixed**: `snapshot_counts` (and every empty-lesson query) crashed on Railway prod with `asyncpg ... cannot get array length of a scalar` — prod has rows where `content_blocks` is a JSON scalar, not an array. Added `_pg.content_blocks_empty_expr()` (a `CASE` guarded by `jsonb_typeof`) and switched `backfill_content.py`, `seed_algomaster_curriculum.py`, `generate_lesson_content.py` to it; the raw SQL in `snapshot_counts` now checks `jsonb_typeof(content_blocks::jsonb) IS DISTINCT FROM 'array'`. Verified the predicate handles `[]`/array/scalar/object/NULL locally.
- **Verified full pipeline green**: `scripts/seed_production.py --yes --skip-backup` against local `algokube` → all 40+ scripts exit 0, algomaster + diagram verify PASS, backfill `Verify: 0 empty`, before/after counts identical (courses=44 sections=338 lessons=1630 empty=0) — zero FAILED entries.

## Session 12 — RICH CONTENT PASS (MERMAID + ANIMATIONS) — COMPLETE
- **Goal met: every diagram-worthy lesson in the 6 algomaster courses now has a hand-authored mermaid diagram + selective problem_header animations.** Applied to BOTH DBs; re-runs are clean no-ops.
- **NEW `scripts/enhance_content_diagrams.py`**: injects curated blocks into existing lessons (never overwrites content, skips any lesson that already has a mermaid block, inserts at the first intro anchor after the opening paragraph/heading). Flags: `--dry-run/--course/--db/--yes/--verify`. Resolves alias source slugs (`ALIAS_SOURCES` mirror of `seed_algomaster_curriculum.ALIASES`) and suffixed slugs (`-course_slug`).
- **NEW 6 modules** (`scripts/content_diagrams_*.py`) with `DIAGRAMS: dict[slug, [blocks]]` — keyed by curriculum lesson slug, value is `[{type:'mermaid', code, title, theme, caption}, optional {type:'problem_header', ...}]`. Themes rotate brand/brand-blue/brand-purple. 610 mermaid diagrams total, ALL validated against mermaid v11 (`node /tmp/opencode/mermaid-check/validate.js` — needs `require('mermaid').default || require('mermaid')` for v11 ESM namespace).
- **Coverage (both DBs, verified)**: ai-engineering 85/87 (97.7%), concurrency-interview 130/132 (98.5%) + 52 problem_header, lld 160/173 (92.5%) + 45 problem_header, microservices 69/71 (97.2%), ml-system-design 51/53 (96.2%), system-design-interviews 111/113 (98.2%). Skipped only roadmap/join-community/quiz lessons.
- **problem_header blocks**: only on algorithmic lessons (concurrency classic problems + design-*; lld design-*). `leetcode_url` included only where a well-known problem exists. Not applied to system-design/microservices/ml (conceptual only).
- Diagram style per course: classDiagram for LLD/OOP, sequenceDiagram for coordination/interaction flows, flowchart for architecture/pipelines, stateDiagram-v2 for lifecycles, erDiagram for data models.
- **Verify**: `seed_algomaster_curriculum.py --verify` still PASSES on both DBs (counts exact, 0 empty). `content_blocks` now served with `mermaid` at intro position (e.g. lld/design-parking-lot nblocks=11, mermaid_at=2).
- Frontend untouched (renderer already supported mermaid/problem_header/array_walkthrough/playground). No tsc/build needed.
...
- `backend/scripts/enhance_content_diagrams.py` — injects curated mermaid/problem_header blocks into existing lessons (idempotent, never overwrites, alias/suffix slug resolution)
- `backend/scripts/content_diagrams_{ai_engineering,ml_system_design,microservices,system_design_interviews,concurrency_interview,lld}.py` — hand-authored `DIAGRAMS: dict[slug, [blocks]]` (610 mermaid + 97 problem_header)
- `backend/scripts/emit_diagrams_json.py` — dumps a `content_diagrams_*` module's DIAGRAMS as JSON (used by the mermaid v11 validator)
