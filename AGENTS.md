...
- **Prod re-run blocker fixed (`seed_production.py`)**: the legacy batch seeder `seed_interview_engineering_courses.py` was still in the MANIFEST for the 5 interview/architecture courses. Re-running it against an algokube-restructured DB violated the globally-unique `lessons.slug` constraint (e.g. `duplicate key ... (slug)=(database-per-service)`), so every prod maintenance run FAILED. Those 5 courses are fully owned by the algokube restructure (`ALGOMASTER_COURSES`, which creates each course from scratch via `seed_algokube_curriculum.py`), so the stale MANIFEST entries were removed and the legacy batch seeder is retired.
- **`--only` validation updated**: `unknown = only - known` now accepts `ALGOMASTER_COURSES` slugs too (they're valid `--only` targets even though not in MANIFEST) — the algokube step handles them end-to-end.
- **Prod scalar `content_blocks` crash fixed**: `snapshot_counts` (and every empty-lesson query) crashed on Railway prod with `asyncpg ... cannot get array length of a scalar` — prod has rows where `content_blocks` is a JSON scalar, not an array. Added `_pg.content_blocks_empty_expr()` (a `CASE` guarded by `jsonb_typeof`) and switched `backfill_content.py`, `seed_algokube_curriculum.py`, `generate_lesson_content.py` to it; the raw SQL in `snapshot_counts` now checks `jsonb_typeof(content_blocks::jsonb) IS DISTINCT FROM 'array'`. Verified the predicate handles `[]`/array/scalar/object/NULL locally.
- **Verified full pipeline green**: `scripts/seed_production.py --yes --skip-backup` against local `algokube` → all 40+ scripts exit 0, algokube + diagram verify PASS, backfill `Verify: 0 empty`, before/after counts identical (courses=44 sections=338 lessons=1630 empty=0) — zero FAILED entries.

## Session 12 — RICH CONTENT PASS (MERMAID + ANIMATIONS) — COMPLETE
- **Goal met: every diagram-worthy lesson in the 6 algokube courses now has a hand-authored mermaid diagram + selective problem_header animations.** Applied to BOTH DBs; re-runs are clean no-ops.
- **NEW `scripts/enhance_content_diagrams.py`**: injects curated blocks into existing lessons (never overwrites content, skips any lesson that already has a mermaid block, inserts at the first intro anchor after the opening paragraph/heading). Flags: `--dry-run/--course/--db/--yes/--verify`. Resolves alias source slugs (`ALIAS_SOURCES` mirror of `seed_algokube_curriculum.ALIASES`) and suffixed slugs (`-course_slug`).
...
- **Verify**: `seed_algokube_curriculum.py --verify` still PASSES on both DBs (counts exact, 0 empty). `content_blocks` now served with `mermaid` at intro position (e.g. lld/design-parking-lot nblocks=11, mermaid_at=2).
