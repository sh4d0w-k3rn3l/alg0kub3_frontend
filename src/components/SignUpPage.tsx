...
import { Code, Sun, Moon, LogIn, Mail, Lock, User, KeyRound, ArrowLeft, AlertCircle, Loader2, Github, Eye, EyeOff } from 'lucide-react';
...
const SignUpPage: React.FC = () => {
  ...
  const handleLinkedInSignUp = ssoSignUp('oauth_linkedin_oidc');
  ...
  useEffect(() => {
        ...
        if (after.status === 'complete') {
          await after.finalize({
            navigate: async ({ session, decorateUrl }) => {
              ...
              if (token) {
                await api.post('/auth/session', { session_id: token, password: '', email: oauthEmail, name: '' }).catch(() => {});
              }
          ...
          setError('Sign up needs a few more details. Please complete the form below.');
        }
      } catch (err: unknown) {
    ...
    return () => ac.abort();
  }, [clerk, clerk.loaded, signUp, router]);
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2.5">
              ...
              <button
                data-testid="linkedin-signup-btn"
